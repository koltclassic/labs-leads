...
class CreateApplication extends Component {
    ...
    uploadFile = async (e) => {
        ...
        data.append('upload_preset', 'labsleads');

        const res = await fetch('https://api.cloudinary.com/v1_1/dphuc9lkc/image/upload', {
            method: 'POST',
            body: data
        });
    ...
    render() {
        return (
            <Mutation mutation={CREATE_APPLICATION_MUTATION} variables={this.state}>
                {(createApp, { loading, error}) => (
                    <Form onSubmit={async e => {
                        ...
                        //change them to the single application page
                        Router.push({
                            pathname: '/application',
                            query: { id: res.data.createApp.id },
                        })
                    }}>
                        <Error error={error} />
